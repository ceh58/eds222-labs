...
mod2 <- lm(hwy~displ + year_cat + displ:year_cat, mpg_new)
summary(mod2)
```

2.  What does the intercept tell you, in words?

    The intercept tells me that when displacement is 0, the hwy mpg is 35 for year = 1999.

3.  What does the coefficient on `displ` tell you, in words?

    The coefficient tells us the change in hwy mpg with change in displacement, year = 1999.

4.  What does the coefficient estimate on the 2008 vintage indicator variable (`year2008`) tell you, in words?

    It tells us the relationship between vintage and hwy mpg when vintage = 2008.
...
```{r}
mpg_new %>%
  ggplot(aes(x = displ, y = hwy, color = year_cat)) +
    geom_point() +
    geom_abline(slope = coef(mod2)[2],
                intercept = coef(mod2)[1],
                color = "coral") +
    geom_abline(slope = (coef(mod2)[2])+coef(mod2)[4],
                intercept = (coef(mod2)[1] + coef(mod2)[3]),
                color = "cyan") +
    #geom_line(data = augment(mod2), aes(y = .fitted, color = year_cat)) + 
...
```{r}
# no interaction
without <- summary(lm(mod))$adj.r.squared
without
# with the interaction
with <- summary(lm(mod2))$adj.r.squared
with
```
...
```{r}
# no interaction
without2 <- summary(lm(mod))$r.squared
without2
# with the interaction
with2 <- summary(lm(mod2))$r.squared
with2
```
