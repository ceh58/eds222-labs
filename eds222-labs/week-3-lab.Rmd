...
```{r}
model_size <- lm(size~latitude, pie_crab)
summary(model_size)
#0.348

model_temp <- lm(water_temp~latitude, pie_crab)
summary(model_temp)
#0.916
```

The regression model for water temperature and latitude fits the data better. This is what I expected based on the scatter plots because the data for water temperature \~ latitude followed a line, where the data for the size \~ latitude was a little more varied.

```{r}
SSR <- sum(model_size$residuals^2)

SST <- sum((pie_crab$size - mean(pie_crab$size))^2) #14.7
...
```{r}
class(mpg$year)
mpg$year_fct <- as.factor(mpg$year)
class(mpg$year_fct)
```
...
mpg %>%
  group_by(year) %>%
  summarize(median(hwy, na.rm = TRUE))

# Medians pretty insensitive to outliers

ggplot(mpg, aes(x = hwy, color = year_fct)) +
  geom_density()
```

The distributions look very similar. The 1999 vintage IQR looks very slightly less fuel efficient as compared to the 2008, but the mean for the 2008 vintage appears to be less than that of the 1999. The bulk of the data overlaps.
...
3.  What does the coefficient estimate on the 2008 vintage indicator variable tell you, in words? **Answers:**
    ...
    Since it is pretty close to 0,

```{r}
summary(lm(hwy~year_fct, mpg))

# B0 23.4274 <- highway mileage at 1999 (when x = 0) value of the REFERENCE LEVEL
# B1 0.0256 <- change/difference between the two (2% change over 9 years) change from REFERENCE LEVEL
