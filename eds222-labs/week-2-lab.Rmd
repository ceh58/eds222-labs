...
# Plot size by latitude
ggplot(data = pie_crab) +
  geom_point(aes(x = latitude, y = size)) +
  theme(aspect.ratio = 1)

ggplot(data = pie_crab) +
  geom_point(aes(x = latitude, y = water_temp)) +
  theme(aspect.ratio = 1)
```
...
(i) the size of crab and latitude: 0.6
(ii) water temperature and latitude: -0.8
...
```{r}
cor(pie_crab$size, pie_crab$latitude) #0.59
cor(pie_crab$water_temp, pie_crab$latitude) #-0.95
```
...
# 2. B0 is mm and B1 is mm/degrees
# 3. B0 is not directly interpretable (can't have a negative size)
```
...
ggplot(data = pie_crab) +
  geom_point(aes(x = latitude, y = size)) +
  #theme(aspect.ratio = 1) +
  geom_smooth(aes(x = latitude, y = size), method = lm, formula = y~x, se = FALSE)
```
